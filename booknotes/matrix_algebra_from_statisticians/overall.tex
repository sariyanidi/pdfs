...
\section*{Notes}
\begin{itemize}
\item Schwarz Inequality: $|\mb{A\cdot B}|\leq ||\mb{A}||\,||\mb{C}||$
...
\mb W & \mb V \\ \mb U & \mb T
\end{bmatrix}$, given that $\mb T$ is non-singular, is computed using the results for block-triangular matrices and the Schur complement.
...
This chapter is about the finding inverses for matrices that are not square or nonsingular. Those so-called generalized inverses apparently become ubiquitous in statistical analysis and possibly other disciplines.
...
\item If $\mb A$ is symmetric, then $(A^-)'$ is a GI of $\mb A$ (a weaker prop compared to nonsingular $(\mb A^{-1})' = \mb A^{-1} $)
\item $\rank(\mb{A^-A}) = \rank(\mb{AA^-}) = \rank(\mb A)$ (C9.3.8)
\item \textbf{L9.3.5}: $\mathcal{C}(\mb B)\subset \mathcal{C}( \mb A) \iff \mb{B=AA^-B}$ and $\mathcal{R}(\mb C)\subset \mathcal{C}( \mb A) \iff \mb{C=CA^-A}$ \\ (A weaker prop compared to nonsingular $\mb{AA^{-1}=I}$ or, equivalently, $\mb{AA^{-1}-I=0}$)
	\begin{itemize}
