...
\input{../preliminaries}
\usepackage{pstricks, pst-node, pst-tree, pst-all}
\begin{document}
...
\begin{itemize}
\item How to compute normalization coefficient for Gaussian distribution? A nice set of tricks does it in eq. 4-28. Try to do it yourself!
\item \textbf{Example 4-12} - Prove Poisson points -  \ie how to show that the probability of number of points in interval is Poisson?
 \item \textbf{Example 4-16} - Prove that if a discrete RV is memoryless then it must be geometric
\item \textbf{Example 4-19} - Nice recap for Bayesian 
\item \textbf{Problem 4-11}
\item \textbf{Problem 4-16}
\item \textbf{Problem 4-20}
\item \textbf{Problem 4-33}
\item \textbf{Problem 4-34} -- But don't spend much time on this.
\end{itemize}
...
%
The equation above is valid for conditional densities too; all we need to do is replace the densities $f_y(y), f_x(x)$ with the conditional densities $f_y(y|M), f_x(x|M)$. Fig. 5-2 is very useful in precisely illustrating the transformation of RV, as well as Fig. 5-1 and Fig. 5-9.
...
\item \textbf{Moments}
	\begin{itemize}
	\item \textit{Moments}: $\mu_n = E\{\mb x^n\}$
	\item \textit{Central moments}: $\mu_n = E\{(\mb x-\mu)^n\}$
	\item \textit{Absolute moments}: $E\{|\mb x-\mu|^n\} \hspace{1truecm} E\{|\mb x|^n\}$
	\item \textit{Generalized moments}: $E\{(\mb x-a)^n\} \hspace{1truecm} E\{|\mb x-a|^n\}$
	\end{itemize}
...
\begin{itemize}
\item \textbf{Chebyshev (Tchebycheff) inequality}: For any $\epsilon >0$, $P\{|x-\mu| \ge \epsilon\} \ge \frac{\sigma^2}{\epsilon^2}$.
\item \textbf{``Inverse'' Tchebycheff inequality}: (I made up this term): For any $\epsilon >0$, $P\{|x-\mu| \le \epsilon\} \le  1-\frac{\sigma^2}{\epsilon^2}$.
...
\item For continuous RVs.
	...
	\item Sum of two poisson distributions is poisson (p.216) and this is easier to show with characteristic function than with manipulation on PMF (compare example 6-33 with example 6-20)
	\item Same as above but for two normal variables (compare example 6-34 with p.175)
...
\begin{itemize}
\item What RV transformation shall we apply to go from $F_x(x)$ to a uniform distribution (p.139)?
\item The same as above but from uniform to $F_y(y)$?
\item Re-read Theorem 5-1 (p.142)
\item Re-do: how to obtain eq. 5-73 -- the $n$th moment of a Gaussian RV?
\item Re-prove: Chebyshev inequality (p.151)
\item Re-prove: Markov inequality (p.151)
\item Re-prove: Bienayme inequality (p.152)
\item Re-prove: Lyapunov inequality (p.152)
\item Re-prove: De Moivre - Laplace theorem (p.156)
\item Re-prove: 5-111 with the elegant trick 
\end{itemize}
...
\section{Re-do}
\begin{itemize}
\item Property 1 (p.170)
\item Marginal statistics (eq 6-9, p.171)
\item Theorem 6-2 (p.174)
\item Circular symmetry; Theorem 6-3 (p.176)
\item Joint normality -> marginal normality (p.175)
\item Circular symmetry (p.181)
\item Example 6-10 (p.186)
\item Example 6-17 (p.193)
\item Equation 6-82 (p.195)
\item Compare direct RV transformation vs transformation via characteristic function; i.e., compare Example 6-20 and Example 6-33
\item Re-do Example 6-36
\item Prove Theorem 6-7: Price's Theorem
\end{itemize}
...
\item \textbf{Orthonormal data transformation: Whitening}. If the data ${\mb x_i}$ are orthogonal, then $R$ is a diagonal matrix. One can obtain an \textit{equivalent} set of linear vectors $\{\mb z_i\}$, in the sense that the vectors of $\mb Z$ and $\mb X$ are related by a linear transformation: $\mb Z = \mb X \Gamma$ (eq. 702). Thus, the covariance matrix of $\mb X$  can be computed in terms of $Z$ (or the opposite):

$$E\{\mb Z^T \mb Z\} = I = E\{\Gamma^T \mb X^T \mb X \Gamma\} = \Gamma^T E\{\mb X^T \mb X\}\Gamma, \text{ therefore (inverse of) cov matrix is: } R^{-1} = \Gamma \Gamma ^T$$

 Whitening is very useful, especially for linear estimators (see also last note in the item list below).
\end{itemize}
...
\item There are several methods for sampling
	...
	\item Some transformations:
		...
		\item Erlang: Sum of $m$ \iid exponentials equals Erlang (\#295)
...
\begin{itemize}
\item Show why sample variance is divided by $n-1$ (Example 7-5)
...
\item Re-read group independence
\item Re-do example 7-7
\item Prove Eq 7-57 (expression for jointly normal vector; p.257)
\item Do example 7-9 (p. 258)
\item Show how to find coefficients $a_i$ in Eq. 7-82
\item Follow argument re: eq 7-95
\item Example 7-12: solve properly
\item Read summary argument about whitening
\end{itemize}
...
% To compile fig below, first compile DVI->PS and then PS->PDF
\begin{figure}[!h]
\begin{center}
