...
\underline{\textit{Example}}. Say, we do the transformation $\mb r = g(\mb{x,y}) = \sqrt{\mb x^2+\mb y^2}$ and $\bs \theta = \tan^{-1}(\mb{y/x})$. Then, in the interval $(-\pi,\pi)$ we have one solution (\ie effectively the inverse transform): $x_1 = r\cos \theta$ and $y_1 = r \sin \theta$. If we use \eqref{eq:j_inverse}, we have: %
